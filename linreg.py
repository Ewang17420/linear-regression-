...
def loss_gradient(X, y, B, lmbda):
    return np.dot(-np.transpose(X),(y - np.dot(X,np.array(B))))

def loss_ridge(X, y, B, lmbda):
    return (y - np.dot(X,np.array(B)))**2 + lmbda * np.dot(B,B)

def loss_gradient_ridge(X, y, B, lmbda):
    return np.dot(-np.transpose(X),(y - np.dot(X,np.array(B)))) + lmbda * np.array(B)
...
def log_likelihood(X, y, B,lmbda):
    ...
    for i in range(len(X)):
        ans += y[i]*X[i]*B[i]-np.log(1+np.exp(X[i]*B[i]))
    return -ans

def log_likelihood_gradient(X, y, B, lmbda):
    return - np.dot(np.transpose(X),(y - sigmoid(np.dot(X,np.array(B)))))
...
def minimize(X, y, loss_gradient,
    ...
    while loss_gradient(X, y, B, lmbda).all() >= precision and ite < max_iter:
...
class RidgeRegression621: # REQUIRED
    ...
    def fit(self, X, y):
        self.B = minimize(X, y,
                          ...
                          addB0=False)
        B0 = y.mean()
        self.B = np.insert(self.B,0,B0,axis=0)
